goog.provide("openmultimedia.utils.TimeFormatter");

openmultimedia.utils.TimeFormatter.formatDuration = function (duration) {
    /** @type {Date} */
    ...
    if ( h > 0 ) {
        ...
        return (m > 9 ? "" : "0") + m + ":" + (s > 9 ? "" : "0") + s + " min";
    }
}

openmultimedia.utils.TimeFormatter.parseTime = function (value) {
    return openmultimedia.utils.TimeFormatter.parseDate(value).time;
}

openmultimedia.utils.TimeFormatter.parseDate = function (value) {
    var pieces = value.match(/^(\d+)-(\d+)-(\d+) (\d+):(\d+)(?::(\d+))?/);
